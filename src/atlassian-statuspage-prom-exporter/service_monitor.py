        ...
        if status_value is not None:
            ...
            if has_cache:
                ...
                if cached_status != status_value:
                    ...
                    logger.debug(f"{service_name}: Status unchanged ({status_value}), but updating gauge to keep metrics fresh in Prometheus")
            else:
            ...
            statuspage_application_timestamp.labels(
                service_name=service_name
            ).set(current_timestamp_ms)
            logger.debug(f"{service_name}: Updated application_timestamp to {current_timestamp_ms}")
            ...
            if has_cache:
                ...
                if current_ids != cached_ids:
                    ...
                    logger.debug(f"{service_name}: Incidents unchanged, but updating gauge to keep metrics fresh in Prometheus")
            else:
                # No cache exists - first run or cache cleared
            ...
            if has_cache:
                ...
                if resolved_ids:
                    ...
                    for resolved_id in resolved_ids:
                        ...
                        logger.debug(f"{service_name}: Set resolved incident {resolved_id} to 0 using cached labels")
            ...
            if incident_metadata:
                ...
                for idx, incident in enumerate(incident_metadata):
                    ...
                    if has_cache and incident_id in cached_ids:
                        ...
                        started_at = normalize_timestamp(cached_inc.get('started_at', 'unknown'))
                        
                        logger.debug(f"{service_name}: Incident {incident_id} exists in cache - using cached labels (updating gauge to keep fresh)")
                    else:
                        ...
                        logger.debug(f"{service_name}: New incident {incident_id} - using current labels")
            ...
            if has_cache:
                ...
                # Build maps for label lookup
                current_by_id = {maint.get('id', 'unknown'): maint for maint in maintenance_metadata}
                ...
                if current_ids != cached_ids:
                    ...
                    logger.debug(f"{service_name}: Maintenance unchanged, but updating gauge to keep metrics fresh in Prometheus")
            else:
                # No cache exists - first run or cache cleared
            ...
            if has_cache:
                ...
                if current_components != cached_components_set:
                    ...
                    logger.debug(f"{service_name}: Components unchanged, but updating gauge to keep metrics fresh in Prometheus")
            else:
                # No cache exists - first run or cache cleared
            ...
            if component_metadata:
                ...
                for idx, component in enumerate(component_metadata):
                    ...
                    logger.debug(f"{service_name}: Set statuspage_component_timestamp to {component_timestamp_ms} for component '{component_name}'")
