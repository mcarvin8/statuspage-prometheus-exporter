...
def check_status_page_service(
    ...
    try:
        ...
        logger.debug(
            f"Status page service {service_key}: Total incidents: {len(all_incidents)}, Active: {len(active_incidents)}, Non-operational components: {len(non_operational_components)}"
        )
        ...
        logger.debug(
            f"Status page service {service_key}: Total maintenances: {len(scheduled_maintenances)}, Active: {len(active_maintenances)}"
        )

        logger.debug(
            f"Status page service {service_key}: Building incident metadata for {len(active_incidents)} active incident(s)"
        )
        ...
        if active_incidents:
            ...
            for inc in active_incidents:
                ...
                if incident_id not in seen_incident_ids:
                    ...
                    logger.debug(
                        f"Status page service {service_key}: Skipping duplicate incident {incident_id}"
                    )

            logger.debug(
                f"Status page service {service_key}: Found {len(active_incidents)} active incident(s), {len(unique_incidents)} unique after deduplication"
            )
            ...
            for inc in unique_incidents:
                ...
                if not shortlink and base_url and incident_id != "unknown":
                    ...
                    logger.debug(
                        f"Status page service {service_key}: Constructed incident URL from ID: {shortlink}"
                    )
                ...
                if name.startswith("_system_metadata:"):
                    logger.debug(
                        f"Status page service {service_key}: Skipping system metadata test incident: {incident_id} ({name[:50]}...)"
                    )
                    continue
                ...
                logger.debug(
                    f"Status page service {service_key}: Incident metadata collected:"
                )
            ...
            logger.debug(
                f"Status page service {service_key}: Found {len(active_incidents)} active incident(s): {description}"
            )
            logger.info(
                f"Status page service {service_key}: Returning {len(incident_metadata)} incident metadata records"
            )
        ...
        if non_operational_components and not active_incidents:
            ...
            indicator = (
                "minor"  # Default to minor if components down but no incident severity
            )
            logger.warning(
        ...
        if active_maintenances:
            logger.debug(
                f"Status page service {service_key}: Building maintenance metadata for {len(active_maintenances)} active maintenance(s)"
            )
            ...
            for maint in active_maintenances:
                ...
                if maintenance_id not in seen_maintenance_ids:
                    ...
                    logger.debug(
                        f"Status page service {service_key}: Skipping duplicate maintenance {maintenance_id}"
                    )

            logger.debug(
                f"Status page service {service_key}: Found {len(active_maintenances)} active maintenance(s), {len(unique_maintenances)} unique after deduplication"
            )

            for maint in unique_maintenances:
                maintenance_id = maint.get("id", "unknown")
                name = maint.get("name", "Unnamed maintenance")
                scheduled_start = maint.get("scheduled_for", "") or maint.get(
                    "created_at", ""
                )
                scheduled_end = maint.get("scheduled_until", "") or maint.get(
                    "scheduled_for", ""
                )
                ...
                logger.debug(
                    f"Status page service {service_key}: Maintenance metadata collected:"
                )
                logger.debug(
                    f"  ID: {maintenance_id}, Name: {name}, Status: {maintenance_status}"
                )
                logger.debug(f"  Scheduled: {scheduled_start} to {scheduled_end}")
                logger.debug(
                    f"  Shortlink: {shortlink}, Affected components: {affected_comps}"
                )

        logger.debug(
            f"Status page service {service_key}: indicator='{indicator}', description='{description}'"
        )
        ...
        logger.debug(
            f"Status page service {service_key}: Final result - status_value={status_value}, status_text='{status_text}'"
        )
        ...
        if existing_cache:
            ...
            existing_maintenance = {
                maint.get("id", "unknown"): maint
                for maint in existing_cache.get("maintenance_metadata", [])
            }
            existing_components = {
                comp.get("name", "Unknown"): comp
                for comp in existing_cache.get("component_metadata", [])
            }
            ...
            for incident in result["incident_metadata"]:
                ...
                if incident_id in existing_incidents:
                    ...
                    incident["name"] = existing_inc.get(
                    ...
                    logger.debug(
                        f"Status page service {service_key}: Preserved original labels for existing incident {incident_id}"
                    )
            ...
            for maintenance in result["maintenance_metadata"]:
                ...
                if maintenance_id in existing_maintenance:
                    ...
                    maintenance["affected_components"] = existing_maint.get(
                        "affected_components",
                        maintenance.get("affected_components", []),
                    )
                    ...
                    logger.debug(
                        f"Status page service {service_key}: Preserved original labels for existing maintenance {maintenance_id}"
                    )
            ...
            for component in result.get("component_metadata", []):
                ...
                if component_name in existing_components:
                    ...
                    logger.debug(
                        f"Status page service {service_key}: Component {component_name} exists in cache, status_value may have changed"
                    )
        ...
        if existing_cache:
            ...
            current_maintenance_ids = {
                maint.get("id", "unknown")
                for maint in result.get("maintenance_metadata", [])
            }
            ...
            current_components = {
                ...
                for comp in result.get("component_metadata", [])
            }
            cached_components = {
                (comp.get("name", "Unknown"), comp.get("status_value", 0))
                for comp in existing_cache.get("component_metadata", [])
            ...
            if not (
                ...
                logger.debug(
                    f"Status page service {service_key}: No meaningful changes detected, skipping cache update"
                )
